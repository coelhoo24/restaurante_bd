-- =====================================================================
-- TESTES DE INTEGRIDADE - SistemaRestaurante
--
-- O QUE É ISSO?
-- Este arquivo não cria nada novo no banco - ele só testa se as regras
-- que já colocamos nas tabelas (PK, FK, CHECK, UNIQUE) e nos objetos
-- (trigger e procedure) estão funcionando de verdade. A ideia é
...
-- Rode o script e olhe a aba "Mensagens" do SSMS (não a de
-- "Resultados"). Cada teste imprime "OK" se o banco se comportou como
-- esperado, ou "FALHOU" se algo passou que não devia. Isso serve como
-- prova, para o professor e para o time, de que a parte de integridade
-- do banco está funcionando.
-- =====================================================================
...
PRINT '=====================================================';
BEGIN TRY
    -- usa o Produto_ID = 1 e Usuario_ID = 1 (ajuste se os seus IDs forem diferentes)
...
-- Pega uma categoria de 'Entrada', mas vamos tentar registrar como 'Saida'
SELECT TOP 1 @IdCategoriaEntrada = ID FROM Categorias_Financeiras WHERE Tipo = 'Entrada';
